<template>
 ...
 <b-form-group class=" mb-2 mt-0" label="Full Name" label-for="name-input" invalid-feedback="Name is required" >
          ...
          <input type="text" class="form-control" placeholder="For 1 Year (Rs. 999.00/-)" aria-label="Recipient's username" aria-describedby="basic-addon2" disabled>
             ...
             <form class="pl-5 pr-5" id="paymentForm" action='https://test.payu.in/_payment' method='post' >
             ...
             <input type="hidden" name="phone" v-model="phone"   size="64"/>
               <input type="hidden" name="lastname"  v-model="users.last_name"  size="64"/>
                <input type="hidden" name="surl" value="http://localhost:3000/PaymentStatus" size="64" />
                <input type="hidden" name="furl" value="http://localhost:3000/home/User/Fail" size="64" />
              <input type="hidden" name="hash" id="hash" v-model="hash" size="64">
               ...
               <b-button @click="paymentcreate();signInButtonPressed();" type="submit" style="background-color: #004898!important" class="  fw-400 ps-3 pe-3 lh-32 mt-3 mb-4 rounded-xxxxl ls-2 bg-success d-inline-block text-white me-1 ms-auto w-100">Create Payment</b-button>
  ...
  methods: {
    ...
    getData() {
      axios.get("user/plan").then((result) => {
        ...
        console.warn("Plan API calling", result);
      });
    },
