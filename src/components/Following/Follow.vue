<template>
  <div>
    <table class="table">
      ...
      <tr
        v-for="(followers, itemindex) in follower"
        v-bind:key="followers.id"
        v-else
      >
        <td class="d-flex ">
          <img
            :src="`${followers.profile_photo}`"
            class="rounded-circle"
            width="55px"
            height="55px"
            v-if="followers.profile_photo"
          />
          <img
          ...
          <h2 class="m-1 ml-3 text-dark font-xss">
            ...
            <span class="d-block font-xssss text-grey-500"
              >{{ followers.city.name }} {{ followers.state.name }}
              {{ followers.country.name }}</span
            >
          </h2>
        </td>
        <td>
          <button
            v-on:click="followRemove(followers, followers.id, index, itemindex)"
            style="float:right; margin:12px 0; height:30px; width:6rem;"
            class="p-1 text-white border-0 rounded-xxxl font-xssss bg-danger"
          >
            Remove
          </button>
        </td>
...
export default {
  ...
  methods: {
    async getData() {
      await axios.get("user-followers/" + this.current_id).then((result) => {
