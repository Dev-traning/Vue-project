<template>
    <div class="border-info-md">
      <div class="pl-4 border-0 card-body w-100 bg-success d-flex rounded-3">
         
         <h4 class="mt-2 mb-0 text-white font-xs fw-600 ms-4">Recieved Quotes for requirements</h4>
      </div>
  ...
  <tbody>
     <tr   v-for="(reciItem,i) in recived" v-bind:key="reciItem.id">
            ...
            <td>{{reciItem.user.first_name}} {{reciItem.user.last_name}}</td>
            <td>{{reciItem.vendor_interest.mobile_no}}</td>
            <td>{{reciItem.vendor_interest.remark}}</td>
            <td>{{reciItem.vendor_interest.rate}}</td>
            <td>
              <router-link :to="'/profile/'+reciItem.user.id"><i class="p-0 feather-eye font-md p-1"></i></router-link>
...
export default {
    ...
    methods:{
 getData() {
      axios.get('restaurants/received-interested-requirements').then((result) => {
