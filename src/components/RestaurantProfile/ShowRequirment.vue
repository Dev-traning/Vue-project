<template>
  <div class="rounded-lg shadow-md mt-2">
    <div class="border-0 card-body w-100 bg-success  rounded-3 ">
      ...
      <div class="row ">
        <div class="col-sm-6">
          <p class="float-start  mb-0 text-white font-xs fw-600 ">
            Requirment list
          </p>
        </div>
        <div class="col-sm-6">
          <router-link
            type="submit"
            to="/PostReq"
            class=" text-primary float-right btn-sm bg-light fw-500"
            >Create Post My Requirment</router-link
          >
        </div>
      </div>
    </div>
    <div class="p-1 border-0 card-body p-lg-3 w-100">
      <table class="table table-bordered">
        ...
        <tbody>
          <tr
            v-for="(requirements, i) in requirement"
          ...
          >
            ...
            <td class="w-25">
              <img
                class="img img-fluid  rounded  w40"
                :src="requirements.photo"
                height="40 rounded"
              />
            </td>
            <td>
              <button
                v-b-modal.edit
                v-on:click="show(requirements.id, i)"
                class="btn p-0 m-0"
                type="button"
              >
                <i class="p-0 feather-edit font-md p-1"></i>
              </button>
              <button
                v-on:click="deletePosts(requirements.id, i)"
                class="btn p-0 m-0"
                type="button"
              >
      ...
      >
        ...
        <form ref="form" @submit.prevent="update">
          <div>
            ...
            >
              <b-form-input
                class=" p-2 ps-2 font-xssss text-grey-500 fw-500 border-light-md "
                id="name-input"
                v-model="sowreq.title"
              ></b-form-input>
            </b-form-group>
            <b-form-group
              class="mt-0 mb-1"
            ...
            >
              <textarea
                name="message"
                class=" p-2 h100 bor-0 w-100 ps-2 font-xsss text-grey-500 fw-500 border-light-md theme-dark-bg "
                cols="30"
                rows="10"
                placeholder="What's on your mind?"
                v-model="sowreq.description"
              ></textarea>
            </b-form-group>
            <div
              class=" p-0 mt-1 card-body border-dark-md h-300 w-100 mb-2 col-lg-12 "
            >
            ...
            <div class="p-0 mt-0 w-100">
              <b-button
                ...
                class=" mt-1 mb-0 text-white w-100 font-xss fw-700 ps-3 pe-3 lh-32 text-uppercase rounded-3 ls-2 bg-success me-1 ms-auto "
                >SAVE</b-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    deletePosts(id, i) {
      axios.delete("restaurants/requirement/" + id).then(() => {
    ...
    async update() {
      ...
      const response = axios
        .post("restaurants/requirement/" + this.itemid, photo)
        .then((result) => {
