<template>
  <div class="container">
    <div
      class="row"
      v-if="this.users.user_type === '6' || this.users.user_type === '7'"
    >
      <div
        class="ml-3 col-12 m-class"
        v-for="item in vendorData.slice(0, 2)"
        v-bind:key="item.id"
      >
        <div
  ...
  methods: {
    getData() {
      ...
      if (this.users.user_type === "6" || this.users.user_type === "7") {
        axios
          .get("vendor/vendor?per_page=2&page=1")
        ...
        axios
          .get("restaurants/job?per_page=2&page=1")
