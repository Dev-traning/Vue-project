...
<div>
   <div class="card w-100 shadow-xss rounded-0 border-0 mb-3 ">
      <div class="card-body d-flex align-items-center p-3 ">
         <h4 class="fw-700 mb-0 font-xssss text-grey-900" v-if="this.users.user_type === '6' || this.users.user_type === '8'"> Manpower </h4>
         ...
         <a   class="fw-600 ms-auto font-xssss text-primary"  v-b-modal.subscribealert v-else>See All</a >   -->
            ...
            <div class="fw-600 ms-auto font-xssss text-primary"  v-if="is_subscribed === user.is_subscribed">
                     <router-link  class="text-primary" v-if="this.users.user_type === '6' || this.users.user_type === '8'" to="/ManpowerShow" >See All </router-link>
                     <router-link  class="text-primary" v-if="this.users.user_type === '7'" :to="'/SMRShow/'+this.user.user_type">See All</router-link>
            </div>
                ...
                <div class="col-12"  v-for="item in manpower" v-bind:key="item.id">
                                 <div class="p-1 mb-3 overflow-hidden bg-white border-0 card w-100 hover-card shadow-xss rounded-xxl" >
                                    <div class="card-image w-100"> 
                                         ...
                                         <img :src="item.user.profile_photo" alt="event" class="w-100" width="250.5" height="187.88" v-if="item.user.profile_photo"/> 
    ...
    getData() {
       this.users = JSON.parse(localStorage.getItem("UserDetails"));
           this.users = JSON.parse(localStorage.getItem("UserDetails"));
            ...
            axios.get("mp/manpowers?per_page=2&page=1")
            .then((result) => {
             this.manpower = result.data.data;
             })}else{axios.get("restaurants/restaurants?per_page=2&page=1")
                .then((result) => {
                 this.manpower = result.data.data})}
