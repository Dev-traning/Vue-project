<template >
  <div>
    <div class="LayerImg" @click.self="hidePopup" v-if="Imagepromet">
      <img
        style="height: 60vh; margin-top: 5%; max-width: 90%"
        :src="ImagesUrlPopup"
      />
    </div>
    <div v-for="(item, itemIndex) in post" v-bind:key="item.id">
      <div class="card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
        <div class="card-body p-0 d-flex">
          <div v-for="(value, key) in item.user" :key="key.id">
            <figure class="avatar me-2" v-if="key === 'profile_photo'">
              ...
              <img
                ...
                v-if="!value"
              />
              <div v-for="(user, index) in item.user" :key="index">
                <div v-for="(innerUser, innerKey) in user.user" :key="innerKey">
                  <template v-if="innerKey == 'blue_tick' && innerUser == 1">
                    <img
                      style="
                        position: absolute;
                        top: 16px;
          ...
          >
            <template #button-content>
              ...
              >
                <i
                  class="p-0 ti-more-alt text-grey-900 btn-round-md bg-greylight font-xss"
                ></i>
              </a>
            </template>
        ...
        <div class="card-body p-0 me-lg-3">
          <p
            class="fw-700 text-grey-700 lh-10 font-xss w-100 mb-0 font-weight-bold"
          >
            {{ item.title }}
          </p>
          ...
          <p
            class="fw-500 text-grey-500 lh-26 font-xssss w-100 mb-2"
            ref="postDescription"
          >
            <span
              v-if="
                !isContentExpanded ||
                (isContentExpanded && item.id !== expandedPostId)
              "
            >
              {{
                isContentExpanded && item.id === expandedPostId
          ...
          <p
            v-if="isContentExpanded && item.id === expandedPostId"
            class="fw-500 text-grey-500 lh-26 font-xssss w-100 mb-2"
          >
            {{ item.description }}
            <span
            ...
            <div class="left">
              <label class="file">
                ...
                <input
                  ...
                  name="images"
                  id="file-small"
                  @change="imageupload('images', $event.target.files)"
                  multiple
                  accept="image/*"
                />
              </label>
  ...
  methods: {
    ...
    getData() {
      ...
      if (this.userDetail.user_type_text == "Restaurant") {
        ...
        axios
          .get("vendor/promotevendors?per_page=5&page=1")
          .then((response) => {
        ...
        axios
          .get("restaurants/promoterestaurants?per_page=5&page=1")
          .then((response) => {
            console.log(response.data);
            this.cards = response.data.data;
            this.titlehead = "nothing";
    ...
    postcomment(_, id, index, itemindex) {
      ...
      axios
        ...
        .then((result) => {
          ...
          let count = this.comments.length;
          this.post[this.post[itemindex].comments_count++].comments_count =
            count;
        });
    },
    async ImgPopup(url) {
