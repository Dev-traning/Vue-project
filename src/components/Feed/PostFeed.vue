<template>
  <div>
    ...
    >
      <div class="card-body p-0 d-flex">
        ...
        <div>
          <h4
            ...
            :key="key.id"
          >
            <router-link :to="'/Profile/' + item.user_id">
              <div v-if="key === 'full_name'" class="text-black">
                {{ value }}
              </div>
            </router-link>
          </h4>
        ...
        >
          <template #button-content>
            ...
            >
              <i
                class="p-0 ti-menu-alt text-grey-900 btn-round-md bg-greylight font-xss"
              ></i>
            </a>
          </template>
      ...
      <div class="row ps-2 pe-2">
        <div
          ...
          v-for="(item, i) in item.medias.slice(0, 3)"
          v-bind:key="item.id"
        >
          <a data-lightbox="roadtri" class="position-relative d-block">
            <a href="#" @click="showImageModalFunction(item.path, itemIndex)">
      ...
      <div class="card-body d-flex p-0">
        <button
          class="btn p-0 d-none-xss d-flex align-items-center fw-600 text-grey-900 text-dark lh-26 font-xssss"
          v-b-modal.likeshome
        ...
        <div>
          ...
          >
            ...
            <b-dropdown-item
              ...
              target="_blank"
            >
              <a class="btn-round-sm bg-facebook"
            ...
            <b-dropdown-item
              ...
              "
              target="_blank"
              ><a class="btn-round-sm bg-twiiter"
            ...
            <b-dropdown-item
              ...
              "
              target="_blank"
              ><a class="btn-round-sm bg-linkedin"
            ...
            >
              <a class="btn-round-sm "
                ><img
                  class="font-xs"
                  src="../../assets/imageedit_3_6192029673.png"
                  alt=""
                />
              </a>
            </b-dropdown-item>
              ...
              <li class="mb-2 media">
                ...
                <div class="p-1 media-body bg-lightgrey fw-500">
                  <div class="d-flex justify-content-between me-0">
                    ...
                    <h5
                      class="mr-2 font-xsss text-grey-500 fw-500 align-items-end"
                    >
                      {{ moment(commentsshow.updated_at).fromNow() }}
                      <b-dropdown
                        v-if="commentsshow.user.id == userid"
                        right
                        size="sm"
                        id="dropdownMenu5"
                        variant="link"
    ...
    <div
      class="card w-100 text-center shadow-xss rounded-xxl border-0 p-2 mb-3 mt-0 "
    >
    ...
    >
      ...
      <form ref="form" @submit.prevent="updatepost" class="p-4 pt-2">
        ...
        <b-button
          ...
          @click="$bvModal.hide('homepageedit')"
          >Post</b-button
        >
      </form>
    </b-modal>
    <b-modal
      class="showImageModal"
      id="modal-center"
      ...
      hide-header
    >
      <div v-for="(post, index) in postMedias" :key="index">
        <a v-if="postCount > 1" href="#" class="previous" @click="previous"
          ><i class="ti-angle-left text-white font-lg"></i
        ></a>
        <img
        ...
        />
        <a v-if="postCount > 1" href="#" class="next" @click="next"
          ><i class="ti-angle-right text-white font-lg"></i
        ></a>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    showImageModalFunction(path, itemIndex) {
      this.visibleSlide = 0;
      this.post_index = itemIndex;
      this.postCount = this.post[this.post_index].medias.length;
      this.postMedias = this.post[this.post_index].medias;
    ...
    previous() {
      if (this.visibleSlide == 0) {
        this.visibleSlide = (this.postCount - 1) % this.postCount;
      } else {
        this.visibleSlide = (this.visibleSlide - 1) % this.postCount;
      }
    },
...
div #modal-center___BV_modal_content_ {
  width: 600px;
}

div #modal-center___BV_modal_body_ .previous {
  position: absolute;
  top: calc(50% - 20px);
}

div #modal-center___BV_modal_body_ .next {
  position: absolute;
  top: calc(50% - 20px);
  right: 27px;
}
